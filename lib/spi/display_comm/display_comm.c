...
LV_IMG_DECLARE(heart1);
LV_IMG_DECLARE(heart2);

static const lv_img_dsc_t * heart_frames[] = {
    &heart1,
    &heart2,
};

static void heart_anim_cb(void * obj, int32_t v) {
    lv_img_set_src(obj, heart_frames[v]);
}

void heart_anim_start(lv_obj_t * img) {
    lv_anim_t a;
    lv_anim_init(&a);
...
void create_widgets(void) {
    ...
    lv_img_set_src(img, &heart1);
    heart_anim_start(img);
